...
creature    *ply_ptr;
{
    ...
    for(obj_tag=ply_ptr->first_obj; obj_tag; obj_tag=obj_next) {
        ...
        if(is_bad_item(obj_tag->obj)) {
            /*del_obj_crt(obj_tag->obj, ply_ptr);*/
            free_obj2(obj_tag->obj);
        }
    } 
}
...
object *obj_ptr;
{
    ...
    for(obj_tag=obj_ptr->first_obj; obj_tag; obj_tag=obj_next) {
        obj_next=obj_tag->next_tag;
        if(is_bad_item(obj_tag->obj)) {
            /* del_obj_obj(obj_tag->obj, obj_ptr); */
            free_obj2(obj_tag->obj);
        }
    } 
}
...
object *obj_ptr;
{
    ...
    if(obj_ptr->armor > 50) {
        ...
        log_dm("나쁜!! %s : %s 방어력 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, obj_ptr->armor);
        else log_dm("나쁜!! ?????? : %s 방어력 : %d\n", obj_ptr->name, obj_ptr->armor);
        return 1;
    }
    if((haha=obj_ptr->ndice*obj_ptr->sdice+obj_ptr->pdice) > 100) {
        if(obj_ptr->parent_crt) 
        log_dm("나쁜!! %s : %s 공격력 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, haha);
        else log_dm("나쁜!! ?????? : %s 공격력 : %d\n", obj_ptr->name, haha);
        return 1;
    }
    if(F_ISSET(obj_ptr, OCONTN) && (obj_ptr->shotsmax > 20)) {
        if(obj_ptr->parent_crt) 
        log_dm("나쁜!! %s : %s 보따리 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, obj_ptr->shotsmax);
        else log_dm("나쁜!! ?????? : %s 보따리 : %d\n", obj_ptr->name, obj_ptr->shotsmax);
        return 1;
    }
    if(F_ISSET(obj_ptr, OSPECI) && (obj_ptr->pdice == 4)) {
        if(obj_ptr->parent_crt) 
        log_dm("나쁜!! %s : %s 이상해 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, obj_ptr->pdice);
        else log_dm("나쁜!! ?????? : %s 이상해 : %d\n", obj_ptr->name, obj_ptr->pdice);
        return 1;
    }
    if((obj_ptr->type == POTION) && (obj_ptr->shotscur > 50)) {
        if(obj_ptr->parent_crt) 
        log_dm("나쁜!! %s : %s 사용회수 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, obj_ptr->shotscur);
        else log_dm("나쁜!! ?????? : %s 사용회수 : %d\n", obj_ptr->name, obj_ptr->shotscur);
        return 1;
    }
    ...
    if(obj_ptr->shotsmax >= 1000) {
        if(obj_ptr->parent_crt) 
        log_dm("나쁜!! %s : %s 최대회수 : %d\n", obj_ptr->parent_crt->name, obj_ptr->name, obj_ptr->shotsmax);
        else log_dm("나쁜!! ?????? : %s 최대회수 : %d\n", obj_ptr->name, obj_ptr->shotsmax);
        return 1;
    }
    return 0;
}
